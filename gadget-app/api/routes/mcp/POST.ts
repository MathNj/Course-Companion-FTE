...
const route: RouteHandler<{ Querystring: { sessionId: string } }> = async ({
  ...
  // Patch the accept header BEFORE creating the transport
  // ChatGPT only sends "text/event-stream" but StreamableHTTPServerTransport requires both
  const originalAccept = request.raw.headers.accept;
  if (originalAccept && !originalAccept.includes("application/json")) {
  ...
  try {
    ...
    if (originalAccept) {
      request.raw.headers.accept = originalAccept;
    }
  } catch (error) {
